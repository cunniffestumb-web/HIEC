...
这是 HIEC 2.0 的独立技术优化版本。原始宣传册、PPT、PDF 和材料源文件均保留在本项目中，便于比赛展示和版本追溯。
...
本地临时公网预览地址不会作为永久网址写入项目，因为它依赖本机进程，关闭服务后会失效。要启用完整 Node 服务，请将本项目上传到 GitHub 后使用 Render 部署，服务根目录选择 `应用`；根目录的 `render.yaml` 已准备好配置。
